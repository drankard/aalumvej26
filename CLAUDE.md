...
- **Content-triggered rebuild is ENABLED** (`EnableContentRebuild` defaults to `true` in `deploy.yml`): re-enabled once the deploy role gained `codebuild:CreateProject`. The resources were absent from the deployed stack between 21 Jul and 1 Aug after two rollbacks.
